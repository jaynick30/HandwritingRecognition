...
public class Perceptron extends PerceptronNet {
	...
	protected double output(int i) {
		return outputs[i];
	}

	protected double error(int i) {
		return errors[i];
	}

	protected double weight(int i, int j) {
		return weights[i][j];
	}

	// Post: error(i) == error
	protected void setError(int i, double error) {
		errors[i] = error;
	}
	...
	public int numInputPaths() {
		return numInputNodes() + 1;
	}
	...
	public void addToWeightDeltas(double[] inputs, double rate) {
		/*
		 * TODO: For each output: For each input: Calculate how this output
		 * wants the weight changed Add that result to the delta[][] for that
		 * weight
		 */
	...
	// Before calling this method, train() has been called for all input/output
	// pairs
