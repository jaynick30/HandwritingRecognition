...
public class MultiLayer extends PerceptronNet {
	...
	public int numInputNodes() {
		return inputToHidden.numInputNodes();
	}

	public int numHiddenNodes() {
		return inputToHidden.numOutputNodes();
	}

	public int numOutputNodes() {
		return hiddenToOutput.numOutputNodes();
	}

	public Perceptron getHiddenLayer() {
	...
	protected void backpropagate(double[] inputs, double rate) {
		/*
		 * TODO: Calculate the backpropagated error for each hidden/output node
		 * pair. Then call inputToHidden.setError() and
		 * inputToHidden.addToWeightDeltas() to store the errors.
		 */
		/*
		 * double inputToHiddenTotal = 0; for(int i = 0; i < numOutputNodes();
		 * i++){ inputToHiddenTotal += inputToHidden.output(i); } double sig =
		 * sigmoid(inputToHiddenTotal); double inputToHiddenGradient =
		 * gradient(sig);
		 */
